...
class StationManager:
    ...
    def __init__(self, config_path: str = None, state_dir: str = None):
        ...
        """
        self.config_path = config_path or os.getenv("STATION_YAML", "./config/station.yaml")
    ...
    def initialize_station(self) -> Station:
        ...
        try:
            ...
            if not config_path.exists():
                logger.warning(f"Station config not found: {config_path}, creating empty station")
    ...
    def load_all_instruments(self) -> Dict[str, bool]:
        ...
        logger.info(f"Load all complete: {success_count}/{len(instrument_names)} instruments loaded")
        
        return results
    
    def autoload_instruments(self, instrument_names: Optional[List[str]] = None) -> Dict[str, bool]:
        ...
        if instrument_names is None:
            ...
            instrument_names = [name.strip() for name in autoload_str.split(",") if name.strip()]
        ...
        success_count = sum(results.values())
        logger.info(f"Autoload complete: {success_count}/{len(instrument_names)} instruments loaded")
    ...
    def get_station_snapshot(self, update: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Station snapshot contains {len(snapshot.get('instruments', {}))} instruments")
            return snapshot
