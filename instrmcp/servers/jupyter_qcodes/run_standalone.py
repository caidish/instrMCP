...
def create_standalone_server() -> FastMCP:
    ...
    async def server_status() -> list[TextContent]:
        ...
        status = {
            ...
            "tools_available": len([name for name in dir(mcp) if not name.startswith('_')]),
            "mock_instruments": list(mock_ipython.user_ns.get('station', {}).get_components() if 'station' in mock_ipython.user_ns else {}),
...
def main():
    ...
    try:
        ...
        print("🔧 Running with STDIO transport for Claude Desktop compatibility", file=sys.stderr)
        print("📋 Available tools: list_instruments, instrument_info, get_parameter_value, list_variables, station_snapshot, server_status", file=sys.stderr)
