...
try:
    from tests.playwright.metadata_client import MCPMetadataClient
except ImportError:
    try:
        from ..metadata_client import MCPMetadataClient  # type: ignore[no-redef]
    except ImportError:
        from metadata_client import MCPMetadataClient  # type: ignore[no-redef]
...
def wait_for_mcp(base_url: str, timeout_s: int = 60) -> bool:
    ...
    Returns:
        True if the server became available, False if timeout
    """
    start = time.time()
    while time.time() - start < timeout_s:
